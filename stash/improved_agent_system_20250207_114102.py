...
DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY') or config.get('deepseek_api_key')
...
OPENROUTER_API_URL = os.getenv('OPENROUTER_API_URL') or config.get('openrouter_api_url')
GROQ_API_URL = os.getenv('GROQ_API_URL') or config.get('groq_api_url')
DEEPSEEK_API_URL = os.getenv('DEEPSEEK_API_URL') or config.get('deepseek_api_url')

DEFAULT_MODEL = config.get('default_model', 'google/gemini-2.0-flash-001')
...
class Agent:
    ...
    def add_job(self, action_xml: str):
        ...
        for action in root.findall('action'):
            ...
            depends_on = [dep.strip() for dep in action.get('depends_on', '').split(',') if dep.strip()] if action.get('depends_on') else []
            ...
            if job_type == 'reasoning':
                model = action.get('model') or DEFAULT_MODEL
    ...
    def process_queue(self):
        """Processes the job queue, executing jobs based on their dependencies."""
        logging.info("Processing job queue...")
    ...
    def _execute_bash_job(self, job: Job):
        """Executes a bash job."""
        logging.info(f"Executing bash job '{job.id}'")
        try:
            ...
            command = job.content
            if not re.match(r'^[a-zA-Z0-9_\-/.\s]+$', command):  # Input validation - very restrictive
                raise ValueError("Invalid characters in bash command")
            ...
            logging.error(f"Error executing bash job '{job.id}': {e}")
            raise

    def _execute_python_job(self, job: Job):
        """Executes a python job."""
        logging.info(f"Executing python job '{job.id}'")
        try:
            ...
            try:
                ...
                # Input validation
                if not re.match(r"^[a-zA-Z0-9_\.\(\)\{\}\[\]\s=+\-*/:,\"']+$", job.content):
                    raise ValueError("Invalid Python code: contains disallowed characters.")
            ...
            logging.error(f"Error executing python job '{job.id}': {e}")
            raise
...
- Include ALL data fields"""
            ...
            logging.error(f"Error executing reasoning job '{job.id}': {e}")
            raise

    def _handle_job_failure(self, job: Job, e: Exception):
        """Handles job failures, logs the error, and updates the job status."""
        logging.exception(f"Job '{job.id}' ({job.type}) failed: {e}")
...
class AgentCLI(Cmd):
    ...
    def _regenerate_with_feedback(self, feedback=None):
        if feedback:
            self.feedback_history.append(feedback)
